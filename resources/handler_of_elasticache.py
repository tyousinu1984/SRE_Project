...
def check_for_elasticache(account_name, config, cloudwatch_client, elasticache_client):
...
def _get_instance_info(config, elasticache_client):
    """elasticacheのclustersを取得

    Parameters
    -----------

    elasticache_client:
        elasticacheのclientオブジェクト
    config: dict
        elasticacheの設定変数

    Returns
    ----------
    instance_info_dict : dict[str, dict]
        elasticacheのクラスター名
    """
    _logger.info(f"{sys._getframe().f_code.co_name} Start.")
    instance_info_dict = {}
